...
public class FocusActivity2Answer : MonoBehaviour
{
    ...
    public void GenerateCubesPerlin()
    {
        ...
        worldOffset.x = Random.Range(0, 100000);
        worldOffset.y = Random.Range(0, 100000);
        treeOffset.x = Random.Range(0, 100000);
        treeOffset.y = Random.Range(0, 100000);
        ...
        for (int x = 0; x < xSize; x++)
        {
            for (int y = 0; y < ySize; y++)
            {
                ...
                if (perlinTreeNoise > treeMinimum && perlinTreeNoise < treeMaximum && perlinNoise >= 0.4f)
                {
                    ...
                    tree.transform.SetParent(worldContainer.transform);
                    allObjects.Add(tree);
                }
