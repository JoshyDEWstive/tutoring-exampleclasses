...
public class GameController : MonoBehaviour
{
    ...
    void Start() {
        ...
        // Gets the value of score and saves in the integer variable
        int ppScore = PlayerPrefs.GetInt("Score");
        if (ppScore != null && ppScore >= 0) score = ppScore;

        // But there is a simpler way of doing this, without using the if statement, we can actually just give the GetInt method a default value if it is not set
        // That means if it is not set, it will give the value we provided as the default
        goal = PlayerPrefs.GetInt("Goal", 10);

        // We need to set the values of the Score and the Goal UI texts at the start of the game, just incase we did save a score from previous
    ...
    IEnumerator RestartGame(GameObject panel)
    {
        ...
        // Updating our high scores
        winHighscoreText.text = "High Score: " + PlayerPrefs.GetInt("HighScore", 0).ToString();
        loseHighscoreText.text = "High Score: " + PlayerPrefs.GetInt("HighScore", 0).ToString();
        Debug.Log("High Score: " + PlayerPrefs.GetInt("HighScore", 0).ToString());
    ...
    public void IncreaseScore() {
        ...
        // To make a high score, we need to get the previous high score and compare it to the current score
        // If the current score is greater than the previous high score then we want to set the high score to the current score
        ...
        // This will save the players score so it can be accessed at another time
        // It takes in the name (or key) of the value and also the value itself
        PlayerPrefs.SetInt("Score", score);
        PlayerPrefs.SetInt("Goal", goal);
